        ...
        if self.game.seven_zero is True:
            ...
            if self.number == 7:
                ...
                if current_player.human is True:
                    ...
                    new_player_name = get_input(
                        "Select a player to switch hands with...", player_name_list
                    )
                    new_player = [
                        player
                        for player in self.game.player_list
...
class Player:
    ...
    def __init__(
        ...
        /,
        *,
        hand: list[BaseUnoCard] | None = None,
    ) -> None:
        self.game = game
        self.name = name
        self.human = human
    ...
    def playable_cards(self) -> list[BaseUnoCard]:
        ...
        for card in self.hand:
            ...
            if (
                isinstance(card, DrawFourCard)
                and isinstance(current, DrawFourCard)
                and self.game.stacking is True
            ):
                playable_cards.append(card)

            elif (
                isinstance(card, NumberedCard)
                and isinstance(current, NumberedCard)
                and card.number == current.number
            ):
                playable_cards.append(card)

            elif card.colour == current.colour and not isinstance(
                card, (WildCard, DrawFourCard)
            ):
                playable_cards.append(card)
    ...
    def get_played_card(self) -> BaseUnoCard:
        if self.human is True:
            ...
            while True:
                ...
                index = int(
                    get_input("Choose the index of the card to play...", index_range)
                )
...
class UnoGame:
        ...
        if new_game is True:
            ...
            for entry in all_data:
                if "Player" in entry:
                    ...
                    cards: list[BaseUnoCard] = [
                        BaseUnoCard.from_string(self, card) for card in cards
                    ]
            ...
            self.deck = [
                BaseUnoCard.from_string(self, card) for card in all_data.get("deck", [])
            ]
            self.pile = [
                BaseUnoCard.from_string(self, card) for card in all_data.get("pile", [])
            ]
    ...
    def save_game(self) -> None:
        ...
        for player in self.player_list:
            player_data = {
                "human": player.human,
                "cards": [repr(card) for card in player.hand],
            }
    ...
    def play_jump_in(self, card: BaseUnoCard, /) -> None:
        for player in self.player_list:
            if player is self.player_from_turn(self.next_turn) and isinstance(
                card, SkipCard
            ):
                continue
            ...
            for player_card in player.hand:
                ...
                if player_card == card:
                    if player.human is True:
                        jump_in = BOOL_MAPPING[
                            get_input(f"{player}, jump in?", BOOL_CHOICES)
                        ]
                    else:
    ...
    def run_game(self) -> None:
        ...
        while True:
            ...
            if not playable_cards:
                ...
                print(
                    f"{current_player} has no playable cards and must draw from the deck."
                )
...
def main() -> None:
    ...
    if not game_data:
        ...
        for entry in game_data:
            ...
            new_game_string = get_input(
                "Do you wish to start a new game?", BOOL_CHOICES
            )
    ...
    if new_game_bool is True:
        ...
        total_player_count = int(
            get_input(
                "How many total players do you want?", [str(i) for i in range(2, 11)]
            )
        )
        human_player_count = int(
            get_input(
                "How many human players do you want?",
